...
function draw() {
    ...
    if (roundFinished === false) {
        ...
        let ratio = 318 / 212
        let imageHeight = windowHeight - 110
        let imageWidth = imageHeight * ratio

        gfx.image(images[1], 0, 0, gfx.width, gfx.height);
        gfx.filter(POSTERIZE, 3);
        ascii_arr = myAsciiArt.convert(gfx);
        myAsciiArt.typeArray2d(ascii_arr, this, 0, 0, (width - imageWidth) / 2, imageHeight);

        gfx2.image(images[0], 0, 0, gfx2.width, gfx2.height);
        gfx2.filter(POSTERIZE, 3);
        ascii_arr2 = myAsciiArt2.convert(gfx2);
        myAsciiArt2.typeArray2d(ascii_arr2, this, (width - imageWidth) / 2, 0, imageWidth, imageHeight);

        myAsciiArt.typeArray2d(ascii_arr, this, (width - imageWidth) / 2 + imageWidth, 0, (width - imageWidth) / 2, imageHeight);
        // setTimeout(() => {
        tint(255, 80);
        image(images[0], (width - imageWidth) / 2, 0, imageWidth, imageHeight);
        noTint();
        // }, 200);
        noLoop()
    }
